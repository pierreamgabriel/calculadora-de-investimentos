...
function calculate() {
	...
	choices.percentage = choices.percentage.replace(/\D/g, "");
	choices.months = choices.months.replace(/\D/g, "");
	choices.amount = choices.amount.replace(/\D/g, "");
	const result = (parseInt(choices.amount) * Math.pow(1 + ((parseFloat(choices.selic) - 0.10) * (parseInt(choices.percentage)/100)/100), parseInt(choices.months)/12)) - parseInt(choices.amount);
